...
from item.club_topic_info_items import ClubTopicInfoScrapyItem
...
class AutoHomeTopicInfoSpider(CrawlSpider):
    ...
    def parse(self, response):
        ...
        yield Request(url=response.url, callback=self.parse_club_topic_info_items, meta={"item": copy.deepcopy(item)}, dont_filter=True)
    ...
    def parse_club_topic_info_items(self, response):
        ...
        if self.club_index < len(self.club_id_list):
            ...
            yield Request(url=url, callback=self.parse_club_topic_info_items, meta={"item": copy.deepcopy(item)},
                          dont_filter=True)
